...
def build_z_fixed(latent_dim):
    ...
    return torch.tensor(z_fixed, device=device, dtype=torch.float, requires_grad=True)
...
model = models.INN_AA(
    ...
    pretrained="../mnist_old/runs_backup/May13_19-39-26_GLaDOS/checkpoints/generator_in.pt",
).to(device)
optimizer = torch.optim.Adam(
    list(filter(lambda p: p.requires_grad, model.parameters())) + [z_fixed], lr=c.lr
)
...
targets = torch.tensor(range(10), device=device).repeat_interleave(c.latent_dim + 2)
fixed_cond_z = [
    fill[:, :, :16, :16][targets],
    fill[:, :, :8, :8][targets],
    make_cond(targets),
...
for epoch in range(c.n_epochs):
    for samples, targets in tqdm(data.train_loader, leave=False, desc=f"{epoch:04d}"):
        optimizer.zero_grad()
        ...
        proto_loss = torch.sum(
            (z_fixed[-1] - torch.mean(sample_latent_mean, dim=0)) ** 2
        )
    ...
    with torch.no_grad():
        ...
        cond = [fill[:, :, :16, :16][y], fill[:, :, :8, :8][y], make_cond(y)]
        t, A, B = model(x, cond)
        recreated, sideinfo = model.sample(A, z_fixed, cond)
        ...
        sample = model.inn(
            torch.randn(100, 32 * 32, device=device), fixed_cond, rev=True
        )
        ...
        if epoch % 10 == 0:
            ...
            for samples, targets in tqdm(data.train_loader, leave=False):
                ...
                if c.interpolation == "linear":
                    ...
                    latent_codes = torch.cat(
                        [latent_codes, (A_ @ z_fixed / np.sin(np.pi * 2 / 3)).cpu()],
                        dim=0,
                    )
            ...
            try:
                ...
                ax.scatter(
                    z_fixed[:, 0].cpu(), z_fixed[:, 1].cpu(), marker="x", s=100, c="k"
                )
                ax.set_aspect("equal")
                fig.colorbar(img)
