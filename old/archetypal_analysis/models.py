...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        t = torch.distributions.multivariate_normal.MultivariateNormal(
            mu, scale_tril=torch.diag_embed(sigma)
        )
...
class Decoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.fc_sideinfo = nn.Sequential(
            nn.Linear(latent_dim, 200), nn.ReLU(), nn.Linear(200, 10), nn.Softmax(dim=1)
        )
...
def INN():
    ...
    def subnet_conv(ch_in, ch_out):
        return nn.Sequential(
            nn.Conv2d(ch_in, 256, 3, padding=1),
            nn.ReLU(),
    ...
    def subnet_conv_1x1(c_in, c_out):
        return nn.Sequential(
            nn.Conv2d(c_in, 256, 1), nn.ReLU(), nn.Conv2d(256, c_out, 1)
        )
    ...
    nodes.append(
        Ff.Node(
            [nodes[-1].out0, split_node.out1], Fm.Concat1d, {"dim": 0}, name="concat"
        )
    )
    ...
    def __init__(
        ...
        if pretrained is not None:
            self.inn.load_state_dict(
                dict(
                    filter(lambda x: "tmp" not in x[0], torch.load(pretrained).items())
                )
            )
