...
inn.load_state_dict(
    dict(
        filter(
            ...
            torch.load(
                "runs/May13_19-39-26_GLaDOS/checkpoints/generator_in.pt"
            ).items(),
        )
    )
)
...
for i in tqdm(range(max_iter)):
    ...
    if i % 500 == 0:
        ...
        with torch.no_grad():
            ...
            writer.add_scalar(
                "disc_training/sample_out", torch.sigmoid(output_out).mean().item(), i
            )
            ...
            writer.add_scalar(
                "disc_training/sample_letter", torch.sigmoid(output).mean().item(), i
            )
            writer.flush()
