...
out_iter = iter(data.fashion_loader)

i = 0

for epoch in range(10):
    for samples, targets in tqdm(data.train_loader):
        ...
        try:
            out_samples, _ = next(out_iter)
        except:
            out_iter = iter(data.fashion_loader)
            out_samples, _ = next(out_iter)

        out_samples = out_samples.to(device)
        ...
        errD_real = F.binary_cross_entropy_with_logits(output_in, ones)
        errD_real.backward()
        ...
        errD_fake = F.binary_cross_entropy_with_logits(output_out, zeros)
        errD_fake.backward()
        ...
        if i % 500 == 0:
            ...
            with torch.no_grad():
                ...
                writer.add_scalar(
                    "disc_training/sample_in", torch.sigmoid(output_in).mean().item(), i
                )
                writer.add_scalar(
                    "disc_training/sample_out",
                    torch.sigmoid(output_out).mean().item(),
                    i,
                )
                ...
                writer.add_scalar(
                    "disc_training/sample_train", torch.sigmoid(output).mean().item(), i
                )
                ...
                writer.add_scalar(
                    "disc_training/sample_test", torch.sigmoid(output).mean().item(), i
                )
                ...
                writer.add_scalar(
                    "disc_training/sample_letter",
                    torch.sigmoid(output).mean().item(),
                    i,
                )
                writer.flush()
