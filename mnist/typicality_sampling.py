...
model = INN().to(device)
model.load_state_dict(torch.load('runs/Jan22_15-00-01_GLaDOS/checkpoints/generator_in.pt'))
...
with torch.no_grad():
    ...
    for i in range(10):
        ...
        d_temp = np.sqrt((extreme_noise - m)[:, None, :] @ icov @
        ...
        samples = model(extreme_noise, make_cond(i * torch.ones(100, device=device).long()), rev=True)

        confidence = torch.nn.functional.softmax(classifier(samples), dim=1)
        confidence_cpu = confidence.cpu()
        corrcoeffs = [np.corrcoef(extreme_noise_cpu[:, j], confidence_cpu[:, i])[0, 1]
