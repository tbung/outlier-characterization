...
def generate_ood_data(N):
    ...
    while ood_samples.shape[0] < N:
        ...
        i = (
            stats.multivariate_normal.pdf(samples, [1, 1], 1 * np.eye(2))
            + stats.multivariate_normal.pdf(samples, [-1, -1], 1 * np.eye(2))
        ) / 2
...
def plot_classifier(model, n_iter):
    ...
    writer.add_figure(
        "Classifier_Pretraining/Boundary", boundaries_fig, n_iter, close=True
    )
    ...
    ax.scatter(x[:, 0], x[:, 1], c=y, cmap="tab10", vmax=9, alpha=0.3, marker=".")
    ...
    writer.add_figure(
        "Classifier_Pretraining/Certainty", certainty_fig, n_iter, close=True
    )
...
def plot_generator(
    ...
    for model, rev in models:
        if rev:
            ...
            fake = model(noise)
        ax.scatter(fake[:, 0].detach().cpu(), fake[:, 1].detach().cpu(), marker=".")
    ax.set_xlim((-10, 10))
    ax.set_ylim((-10, 10))
    ax.set_aspect("equal")
...
def plot_discriminator(model, n_iter):
    ...
    fig, ax = plt.subplots()
    ax.contourf(xx, yy, zz.detach().cpu().numpy().reshape(1000, 1000), cmap="inferno")
    ax.scatter(x[:, 0], x[:, 1], c=y, cmap="tab10", vmax=9, marker=".")
    ...
    writer.add_figure("GAN_Training/Discriminator", fig, n_iter, close=True)
...
def pretrain_classifier(restore=True):
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=c.lr_classifier, weight_decay=1e-4
    )
    ...
    for epoch in range(c.n_epochs_pretrain):
        ...
        for i, (x, y) in enumerate(test_loader):
            ...
            outputs = torch.argmax(F.softmax(model(inputs), dim=1), dim=1)
...
train_loader = data.DataLoader(
    data.TensorDataset(*generate_toy_data(N=10000)),
    batch_size=c.batch_size,
    shuffle=True,
)

test_loader = data.DataLoader(
    data.TensorDataset(*generate_toy_data(N=1000)),
    batch_size=c.batch_size,
    shuffle=False,
)
...
scheduler_D = torch.optim.lr_scheduler.StepLR(optimizer_D, c.lr_schedule, c.lr_step)
