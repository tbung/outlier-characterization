...
def generate_image(true_dist, epoch):
    ...
    plt.savefig(f"both_inn/discriminator_{epoch:08d}.png", figsize=(8, 8))
    plt.close()
...
def train_classifier():
    ...
    for epoch in range(20):
        ...
        if epoch % 10 == 0:
            ...
            ax = plt.gca()
            ax.set_aspect("equal")
            plt.savefig(f"both_inn/classifier_{epoch:03d}.png", figsize=(8, 8))
            plt.close()
            ...
            ax = plt.gca()
            ax.set_aspect("equal")
            plt.savefig(f"both_inn/confidence_{epoch:03d}.png", figsize=(8, 8))
            plt.close()
...
optimizerG = torch.optim.Adam(inn_out.parameters(), lr=LR, betas=(0.5, 0.999))
optimizerGin = torch.optim.Adam(inn_in.parameters(), lr=LR, betas=(0.5, 0.999))
decreasing_lr = 60
...
def train(epoch):
    ...
    for batch_idx, (d, target) in enumerate(dataloader):
        ...
        jac = inn_out.jacobian(run_forward=False)
        ...
        output = inn_in(d)
        zz = torch.sum(output ** 2, dim=1)
        jac = inn_in.jacobian(run_forward=False)
    ...
    ax = plt.gca()
    ax.set_aspect("equal")
    plt.savefig(f"both_inn/generator_{epoch:08d}.png", figsize=(8, 8))
    plt.close()
    ...
    ax = plt.gca()
    ax.set_aspect("equal")
    plt.savefig(f"both_inn/classifier_{epoch:08d}.png", figsize=(8, 8))
    plt.close()
    ...
    ax = plt.gca()
    ax.set_aspect("equal")
    plt.savefig(f"both_inn/confidence_{epoch:08d}.png", figsize=(8, 8))
    plt.close()
