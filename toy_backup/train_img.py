...
parser.add_argument(
    "--beta", type=float, default=1, help="penalty parameter for KL term"
)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        uniform_dist = torch.Tensor(data.size(0), args.num_classes).fill_(
        ...
        data, target, uniform_dist = (
            Variable(data),
            Variable(target),
            Variable(uniform_dist),
        )
        ...
        if batch_idx % args.log_interval == 0:
            print(
                "Classification Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, KL fake Loss: {:.6f}".format(
                    epoch,
                    batch_idx * len(data),
            ...
            vutils.save_image(
                fake.data,
                "%s/gan_samples_epoch_%03d.png" % (args.outf, epoch),
                normalize=True,
            )
